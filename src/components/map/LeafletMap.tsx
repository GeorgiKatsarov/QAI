...
const cityCoords: Record<string, [number, number]> = {
  ...
  Ruse:            [43.8480, 25.9540],
  "Stara Zagora":  [42.4258, 25.6345],
  Pleven:          [43.4168, 24.6069],
  Blagoevgrad:     [42.0139, 23.0942],
};
...
export function LeafletMap({ events, onSelectEvent }: Props) {
  ...
  useEffect(() => {
    ...
    import("leaflet").then((mod) => {
      ...
      // Soft pastel tile layer (Stadia Alidade Smooth)
      L.tileLayer(
  ...
  // Update markers when events change without rebuilding the map
  useEffect(() => {
    ...
    import("leaflet").then((mod) => {
      const L = mod.default;
      // Remove old marker layers (keep tile layer)
      map.eachLayer((layer) => {
...
function addMarkers(
  ...
  events.forEach((event, i) => {
    ...
    // Tag so we can remove them later
